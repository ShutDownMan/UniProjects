...
void Processor::initialize(unsigned char *memoryRef) {
    ...
    this->memory->initialize(memoryRef,
                             new INBus(this->IorDMux->getOutBus()),
...
void Processor::clock() {
    Machine::debugInfo("Clocking Component: PC Register", 1);
    this->pc->updateState();
    ...
    Machine::debugInfo("Clocking Component: Instruction Register", 1);
    this->instructionRegister->updateState();

    Machine::debugInfo("Clocking Component: Memory Data Register", 1);
    this->memoryDataRegister->updateState();

    Machine::debugInfo("Clocking Component: Registers", 1);
    this->registers->updateState();
    this->registers->printContents();

    Machine::debugInfo("Clocking Component: Register A", 1);
    this->registerA->updateState();

    Machine::debugInfo("Clocking Component: Register B", 1);
    this->registerB->updateState();

    Machine::debugInfo("Clocking Component: ALU Out Register", 1);
    this->aluOut->updateState();
}
...
//    pcSourceMux->printContents();
    ...
    Machine::debugInfo("Updating IO of Component: IorD Multiplexer", 1);
    this->IorDMux->updatePassive();
    this->IorDMux->printContents();

    Machine::debugInfo("Updating IO of Component: Memory to Register Multiplexer", 1);
    this->memToRegMux->updatePassive();
//    this->memToRegMux->printContents();
}
